...
import { getAssetTypes, createAssetType, updateAssetType, deleteAssetType, getAssetCategories } from '@/actions/admin/assetActions';
import { toast } from "sonner";
import { Pencil, Trash2, Plus, Loader2, Tag, Search, X, Check, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function ManageAssetTypesModal({ open, onOpenChange, onUpdate }: ManageAssetTypesModalProps) {
    ...
    const handleView = (type: any) => {
        setCurrentType(type);
        setName(type.asset_type_name);
        setDesc(type.asset_type_desc || "");
        setCategoryId(type.asset_category_id?.toString() || "");
        setStatus(type.asset_type_status ?? 1);
        setMode('view');
    };
    ...
    const handleBack = () => {
        if (mode === 'edit') {
            if (currentType) {
                setName(currentType.asset_type_name);
                setDesc(currentType.asset_type_desc || "");
                setCategoryId(currentType.asset_category_id?.toString() || "");
                setStatus(currentType.asset_type_status ?? 1);
                setMode('view');
            } else {
                setMode('list');
            }
    ...
    const handleSave = async () => {
        ...
        const payload = {
            ...
            asset_type_desc: desc,
            asset_category_id: parseInt(categoryId),
            asset_type_status: status
        };

        let res;
        if (currentType && (mode === 'edit' || mode === 'view')) {
            res = await updateAssetType(currentType.asset_type_id, payload);
        } else {
            res = await createAssetType(payload);
        }
    ...
    const onConfirmDelete = async () => {
        ...
        const res = await deleteAssetType(currentType.asset_type_id);
        if (res.success) {
    ...
    const filteredTypes = types.filter(type =>
        type.asset_type_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (type.asset_type_desc && type.asset_type_desc.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    const getCategoryName = (id: number) => {
        const cat = categories.find(c => c.asset_category_id === id);
        return cat ? cat.asset_category_name : 'Unknown';
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
                ...
                <div className="relative">
                    ...
                    <DialogHeader className="relative z-10 p-6 pb-2 text-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <DialogTitle className="text-2xl font-heading text-white">Manage Types</DialogTitle>
                                <p className="text-white/70 text-sm">Create and manage asset types</p>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    {mode === 'list' && (
                        <div className="relative z-10 px-6 pb-6 pt-2">
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <Button onClick={handleAddNew} className="bg-white text-pink-600 hover:bg-white/90 border-0 shadow-lg shadow-pink-900/20 font-semibold gap-2">
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                placeholder="e.g. Apartment, Bitcoin..."
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Textarea
                                            ...
                                            placeholder="Description of this asset type..."
                                    ...
                                    <div className="divide-y divide-border">
                                        {paginatedTypes.map((type) => (
                                            ...
                                            >
                                                ...
                                                <div className="w-[20%] text-sm text-foreground/80 truncate">
                                                    {getCategoryName(type.asset_category_id)}
                                                </div>
                                                <div className="w-[35%] text-sm text-muted-foreground truncate">
                                                    {type.asset_type_desc || <span className="opacity-50 italic">No description</span>}
                                                </div>
                                                <div className="w-[20%] flex justify-center">
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${type.asset_type_status === 1
                                                        ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'
                                                        : 'bg-red-500/10 text-red-600 border-red-500/20'
                                                        }`}>
                                                        {type.asset_type_status === 1 ? 'Active' : 'Inactive'}
                                                    </span>
                                                </div>
                                            </div>
                                        ))}
            ...
            <AlertModal
                ...
                description="Are you sure you want to delete this asset type?"
            />
        </Dialog>
    );
}
