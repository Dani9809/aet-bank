...
import { getInvestmentCategories, createInvestmentCategory, updateInvestmentCategory, deleteInvestmentCategory } from '@/actions/admin/investmentActions';
...
interface ManageInvestmentCategoriesModalProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    onUpdate: () => void;
}

export function ManageInvestmentCategoriesModal({ open, onOpenChange, onUpdate }: ManageInvestmentCategoriesModalProps) {
    ...
    const fetchCategories = useCallback(async () => {
        setLoading(true);
        const res = await getInvestmentCategories();
    ...
    const handleView = (category: any) => {
        ...
        setName(category.investment_category_name);
        setDesc(category.investment_category_desc || "");
        setFocus(category.investment_category_focus || "");
        setStatus(category.investment_category_status ?? 1);
        setMode('view');
    };
    ...
    const handleBack = () => {
        if (mode === 'edit') {
            if (currentCategory) {
                setName(currentCategory.investment_category_name);
                setDesc(currentCategory.investment_category_desc || "");
                setFocus(currentCategory.investment_category_focus || "");
                setStatus(currentCategory.investment_category_status ?? 1);
                setMode('view');
            } else {
                setMode('list');
            }
    ...
    const handleSave = async () => {
        ...
        const payload = {
            investment_category_name: name,
            investment_category_desc: desc,
            investment_category_focus: focus,
            investment_category_status: status
        };

        let res;
        if (currentCategory && (mode === 'edit' || mode === 'view')) {
            res = await updateInvestmentCategory(currentCategory.investment_category_id, payload);
        } else {
            res = await createInvestmentCategory(payload);
        }
    ...
    const onConfirmDelete = async () => {
        ...
        const res = await deleteInvestmentCategory(currentCategory.investment_category_id);
        if (res.success) {
    ...
    const filteredCategories = categories.filter(cat =>
        cat.investment_category_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (cat.investment_category_desc && cat.investment_category_desc.toLowerCase().includes(searchTerm.toLowerCase()))
    );
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[95vw] max-w-4xl bg-card border-border max-h-[90vh] flex flex-col p-0 overflow-hidden rounded-2xl shadow-2xl [&>button]:hidden">
                ...
                <div className="relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 via-purple-500 to-amber-500" />
                    ...
                    <DialogHeader className="relative z-10 p-6 pb-2 text-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <DialogTitle className="text-2xl font-heading text-white">Manage Investment Categories</DialogTitle>
                                <p className="text-white/70 text-sm">Create and manage investment categories</p>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    {mode === 'list' && (
                        <div className="relative z-10 px-6 pb-6 pt-2">
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <Button onClick={handleAddNew} className="bg-white text-purple-600 hover:bg-white/90 border-0 shadow-lg shadow-purple-900/20 font-semibold gap-2 font-source tracking-tight">
                                    <Plus className="h-4 w-4" /> Add Application Category
                                </Button>
                            </div>
                        </div>
                    )}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            placeholder="e.g. Growth, Stability, Dividend..."
                                            className="bg-background disabled:opacity-100 disabled:bg-muted/10"
                                            disabled={mode === 'view'}
                                        />
                                    </div>
                                ...
                                {loading ? (
                                    ...
                                    <div className="divide-y divide-border">
                                        {paginatedCategories.map((cat) => (
                                            <div
                                                key={cat.investment_category_id}
                                                className="flex items-center px-6 py-4 hover:bg-muted/20 transition-colors cursor-pointer group gap-4"
                                                onClick={() => handleView(cat)}
                                            >
                                                <div className="w-[25%] font-medium text-foreground truncate group-hover:text-primary transition-colors">
                                                    {cat.investment_category_name}
                                                </div>
                                                <div className="w-[25%] text-sm text-foreground/80 truncate">
                                                    {cat.investment_category_focus || <span className="opacity-50 italic">N/A</span>}
                                                </div>
                                                <div className="w-[30%] text-sm text-muted-foreground truncate">
                                                    {cat.investment_category_desc || <span className="opacity-50 italic">No description</span>}
                                                </div>
                                                <div className="w-[20%] flex justify-center">
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${cat.investment_category_status === 1
                                                        ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'
                                                        : 'bg-red-500/10 text-red-600 border-red-500/20'
                                                        }`}>
                                                        {cat.investment_category_status === 1 ? 'Active' : 'Inactive'}
                                                    </span>
                                                </div>
                                            </div>
                                        ))}
