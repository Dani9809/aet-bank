...
import { getBusinessCategories, createBusinessCategory, updateBusinessCategory, deleteBusinessCategory } from '@/actions/admin/businessActions';
...
export function ManageCategoriesModal({ open, onOpenChange, onUpdate }: ManageCategoriesModalProps) {
    ...
    const handleView = (category: any) => {
        ...
        setName(category.business_category_name);
        setDesc(category.business_category_desc || "");
        setStatus(category.business_category_status ?? 1);
        setMode('view');
    };
    ...
    const handleBack = () => {
        if (mode === 'edit') {
            ...
            if (currentCategory) {
                setName(currentCategory.business_category_name);
                setDesc(currentCategory.business_category_desc || "");
                setStatus(currentCategory.business_category_status ?? 1);
                setMode('view');
            } else {
                setMode('list');
            }
    ...
    const handleSave = async () => {
        ...
        const payload = {
            ...
            business_category_desc: desc,
            business_category_status: status
        };

        let res;
        if (currentCategory && (mode === 'edit' || mode === 'view')) { // view check just in case, primarily edit
            res = await updateBusinessCategory(currentCategory.business_category_id, payload);
        } else {
            res = await createBusinessCategory(payload);
        }

        if (res.success) {
            toast.success(mode === 'create' ? "Category created" : "Category updated");

            // If created, maybe go to list or view? Let's go to list to see it.
            // If updated, go back to view mode with new data
            if (mode === 'edit') {
    ...
    const handleDelete = async () => {
        ...
        if (confirm("Are you sure you want to delete this category?")) {
            const res = await deleteBusinessCategory(currentCategory.business_category_id);
            if (res.success) {
    ...
    const filteredCategories = categories.filter(cat =>
        cat.business_category_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (cat.business_category_desc && cat.business_category_desc.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[95vw] max-w-4xl bg-card border-border max-h-[90vh] flex flex-col p-0 overflow-hidden rounded-2xl shadow-2xl [&>button]:hidden">
                    ...
                    <DialogHeader className="relative z-10 p-6 pb-2 text-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-white/70 text-sm">Create and manage business categories</p>
                            </div>
                        </div>
                    </DialogHeader>
                ...
                {mode !== 'list' ? (
                    // ... (Edit/Create/View mode content - unchanged)
                    <>
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            placeholder="e.g. Retail, Technology, Service..."
                                        ...
                                        {paginatedCategories.map((cat) => (
                                            ...
                                            >
                                                ...
                                                <div className="w-[50%] text-sm text-muted-foreground truncate">
                                                    {cat.business_category_desc || <span className="opacity-50 italic">No description</span>}
                                                </div>
                                                <div className="w-[20%] flex justify-center">
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${cat.business_category_status === 1
                                                        ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'
                                                        : 'bg-red-500/10 text-red-600 border-red-500/20'
                                                        }`}>
                                                        {cat.business_category_status === 1 ? 'Active' : 'Inactive'}
                                                    </span>
                                                </div>
                                            </div>
                                        ))}
